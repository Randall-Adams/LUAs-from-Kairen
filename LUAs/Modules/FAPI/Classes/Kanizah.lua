...
_G.Kanizah = {
            ...
            while nl ~= nil do
                if nl ~= "" then
                    ...
                    elseif nl == "UpdatePlayerData" then
                        ...
                        local PlayerName = FAPI.IO.ReadNextLine(FDi, cs)
                        print("(" .. PlayerName .. ")")
                        local PlayersX = FAPI.IO.ReadNextLine(FDi, cs)
                        local PlayersY = FAPI.IO.ReadNextLine(FDi, cs)
                        local PlayersZ = FAPI.IO.ReadNextLine(FDi, cs)
                        local PlayersF = FAPI.IO.ReadNextLine(FDi, cs)
                        local thisPlayer
                        thisPlayer = NPCs.Find("Name", PlayerName, "NPC")
                        --SilentMode = false;
                        if thisPlayer ~= nil then
                            print("Found")
                            thisPlayer.X(PlayersX)
                            thisPlayer.Y(PlayersY)
                            thisPlayer.Z(PlayersZ)
                            thisPlayer.F(PlayersF)
                        else
                            print("Unfound")
                        end
                    ...
                    elseif nl == "SetLanPlayFolder" then
                        RF.EQOA.Net_Streams.o.LanPlayOutput.self = FAPI.IO.ReadNextLine(FDi, cs)
                        SilentMode = false
                        print("Setting Lan Play Folder to: " .. RF.EQOA.Net_Streams.o.LanPlayOutput.self)
                        SilentMode = true

                    elseif nl == "SetLanPlayerName" then
                        local number
                        local name
                        number = FAPI.IO.ReadNextLine(FDi, cs);
                        name = FAPI.IO.ReadNextLine(FDi, cs);
                        if number == "1" then
                            number = 1
                        ...
                        LanPlayers[number] = name
                        SilentMode = false
                        print("Setting Lan Player Name " .. number .. " to " .. name)
                        local newnpcindex
                        newnpcindex = NPCs.Spawn("Randnull_Adams", "NPC")
                        if newnpcindex ~= nil then
                            newnpcindex.Name(name)
                        end
                        SilentMode = true

                    elseif nl == "SetMyLanPlayerNumber" then
                        local number
                        number = FAPI.IO.ReadNextLine(FDi, cs);
                        if number == "1" then
                            number = 1
                        ...
                        LanPlayers[0] = number
                        SilentMode = false
                        print("Setting My Lan Player Number to " .. number)
                        SilentMode = true

                    elseif nl == "SetMyLanPlayerName" then
                        local name
                        name = FAPI.IO.ReadNextLine(FDi, cs);
                        --SilentMode = false
                        print("Setting My Lan Player Name to " .. name)
                        --SilentMode = true
                        EQOA.Player.Profile.Name.Value(name)

                    elseif nl == "CloseAll" then
                        c()

                    elseif nl == "CreateLANPlayer" then
                        local LANPlayer = {};
                        LANPlayer.Name = FAPI.IO.ReadNextLine(FDi, cs);
                        LANPlayer.X = FAPI.IO.ReadNextLine(FDi, cs);
                        LANPlayer.Y = FAPI.IO.ReadNextLine(FDi, cs);
                        LANPlayer.Z = FAPI.IO.ReadNextLine(FDi, cs);
                        LANPlayer.F = FAPI.IO.ReadNextLine(FDi, cs);
                        FAPI.Kanizah.CreateNPCFile(LANPlayer.Name, LANPlayer.Name, nil, nil, nil, LANPlayer.X, LANPlayer.Y, LANPlayer.Z, LANPlayer.F, RF.EQOA.self .. "Temp Data/");
                        -- RF.EQOA.self .. "Temp Data/"
                        NPCs.Spawn(nil, "NPC", LANPlayer.Name)

                    elseif nl == "GetRunOptions" then
                        local ROOutput = io.open(RF.EQOA.Net_Streams.o.self .. "FAPI Data Request" .. Extension_ReadWrites,"w+")
                        if ROOutput ~= nil then
                            for key,value in pairs(FAPI.RunOptions) do
                                if key ~= "New" then
                                    ROOutput:write(key)
                                    ROOutput:write("\n");
                                    ROOutput:write(tostring(value.Do))
                                    ROOutput:write("\n");
                                end
                            end
                        end
                    ...
                    elseif nl == "SetRunOption" then
                    --SilentMode = false
                    print("Setting RunOption")
                    local lenext1 = FAPI.IO.ReadNextLine(FDi, cs)
                    print("Looking for [" .. lenext1 .. "]")
                        for key,value in pairs(FAPI.RunOptions) do
                        print("Found Key [" .. key .. "]")
                            if key == lenext1 then
                                print("Desired Key Found")
                                local lenext20 = FAPI.IO.ReadNextLine(FDi, cs)
                                print("Setting to " .. lenext20)
                                local lenext2 = FAPI.HelperFunctions.Convert_StringToBoolean(lenext20)
                                print("Setting to " .. FAPI.HelperFunctions.Convert_BooleanToString(lenext2))
                                value.Do = lenext2
                            end
                        end
                        print("Done")
                    ...
                    elseif nl == "TestMoveScript" then
                        --SilentMode = false
                        dofile(RF.EQOA.LUAs.self .. "MoveScripts/" .. FAPI.IO.ReadNextLine(FDi, cs))

                        print("Tested MoveScript")
                    else
        ...
        local nl
            ...
            while nl ~= nil do
                if nl ~= "" then
                    ...
                    elseif nl == "UpdatePlayerData" then
                        print("Kanizah.UpdatePlayerData()")
                        local PlayerName = FAPI.IO.ReadNextLine(FDi_Unreliable, cs)
                        print("(" .. PlayerName .. ")")
                        local PlayersX = FAPI.IO.ReadNextLine(FDi_Unreliable, cs)
                        local PlayersY = FAPI.IO.ReadNextLine(FDi_Unreliable, cs)
                        local PlayersZ = FAPI.IO.ReadNextLine(FDi_Unreliable, cs)
                        local PlayersF = FAPI.IO.ReadNextLine(FDi_Unreliable, cs)
                        local thisPlayer
                        thisPlayer = NPCs.Find("Name", PlayerName, "NPC")
                        if thisPlayer ~= nil then
                            print("Found")
                    ...
                    elseif nl == "CreateLANPlayer" then
                        local LANPlayerName -- = {};
                        LANPlayerName = FAPI.IO.ReadNextLine(FDi_Unreliable, cs);
                        --LANPlayer.X = FAPI.IO.ReadNextLine(FDi_Unreliable, cs);
                        --LANPlayer.Y = FAPI.IO.ReadNextLine(FDi_Unreliable, cs);
                        --LANPlayer.Z = FAPI.IO.ReadNextLine(FDi_Unreliable, cs);
                        --LANPlayer.F = FAPI.IO.ReadNextLine(FDi_Unreliable, cs);
                        --FAPI.Kanizah.CreateNPCFile(LANPlayer.Name, LANPlayer.Name, nil, nil, nil, LANPlayer.X, LANPlayer.Y, LANPlayer.Z, LANPlayer.F, RF.EQOA.self .. "Temp Data/");
                        -- RF.EQOA.self .. "Temp Data/"
                        NPCs.Spawn(nil, "NPC", RF.EQOA.self .. "Temp Data/" .. LANPlayerName .. Extension_SpawnPoints)
    ...
    UpdateVariableByVariableName = (function(_variableNewValue, _variableToUpdate)
        --print("Updating " .. _variableToUpdate .. " with " .. _variableNewValue)
        for key,value in pairs(_G) do
