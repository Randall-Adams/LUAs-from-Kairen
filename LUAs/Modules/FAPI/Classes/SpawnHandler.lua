...
NPC = {};
NPC[0] = {};
NPC[0].Name = readBytes("[[pcsx2-r3878.exe+003FEE7C]+8A4]", 24, true);
NPC[0].x = readFloat("[[pcsx2-r3878.exe+00400868]+650");
NPC[0].y = readFloat("[[pcsx2-r3878.exe+00400868]+658");
NPC[0].z = readFloat("[[pcsx2-r3878.exe+00400868]+654");
NPC[0].f = readFloat("[[pcsx2-r3878.exe+00400868]+610");
NPC[1] = {};
NPC[1].Name = "Bob111";
NPC[1].x = readFloat();
NPC[1].y = readFloat();
NPC[1].z = readFloat();
NPC[1].f = readFloat();
for index1,value1 in pairs(NPC) do
    for index2,value2 in pairs(value1) do
        print(index2 .. "| " .. value2)
    end
end
readBytes(BitArrayAddress, LengthtoRead, true)
...
_G.SpawnHandler = {
    NewSpawnPoint = (function(_nameSafe)
      --print("Creating Spawn Point -- " .. _nameSafe)
        ...
        if _G.FAPI.IO.File_Exists(RF.EQOA.Custom_Data.Spawn_Points.self .. _nameSafe .. Extension_SpawnPoints) == true then
            self.FilePath = RF.EQOA.Custom_Data.Spawn_Points.self .. _nameSafe .. Extension_SpawnPoints
        elseif _G.FAPI.IO.File_Exists(RF.EQOA.Game_Data.Spawn_Points.self .. _nameSafe .. Extension_SpawnPoints) == true then
            self.FilePath = RF.EQOA.Game_Data.Spawn_Points.self .. _nameSafe .. Extension_SpawnPoints
        else
